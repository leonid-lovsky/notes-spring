...
> **Назначение:** снимок итогов сессии 2026-09-26 по микросервисной архитектуре — предпочтения пользователя, основной вопрос, сервисы, Spring Cloud, где security/валидация/ошибки/технологии, выбор технологии при запуске, границы
> **Когда читать:** первым при возврате к микросервисам, сервисам, Spring Cloud, границам модулей/сервисов
> **Статус:** консультации ассистента 2026-09-26, решения пользователем не приняты, в коде ничего нет; открытое — раздел «Открыто (решает пользователь)»
> **Разделы:** «Предпочтения пользователя…», «⚠️ Основной вопрос…», «Сервисы и ответственность», «Классический набор Spring Cloud…», «Компоненты Spring Cloud — подробности», «Модель `UserNote` без прав», «Где реализуется Security/валидация/обработка ошибок/технологии», «Выбор технологии при запуске», «Границы модулей и сервисов», «Открыто»
> **Связано:** `docs/user-note-access-model.md` (ход рассуждения и код-эскизы), CLAUDE.md → «Открытые решения» → «⚠️ Основной вопрос микросервисной архитектуры», «Межсервисная консистентность»
