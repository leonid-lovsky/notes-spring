# Инструменты миграции БД по адаптерам

> **Назначение:** обзор инструментов миграции/версионирования схемы для каждого возможного driven-адаптера (Flyway/Liquibase, Mongock/Flamingock, elasticsearch-evolution)
> **Когда читать:** при выборе инструмента миграции или появлении схем `schema-<vendor>.sql` под версионирование
> **Статус:** справочный обзор 2026-07-15, ничего не подключено, выбор не сделан (CLAUDE.md → «Открытые решения» → «Инструмент миграции БД по адаптерам»); ключевой факт — Mongock в maintenance mode, EOL конец 2026, преемник Flamingock
> **Разделы:** по адаптерам — `data-jpa · data-jdbc`, `data-r2dbc`, `data-mongodb`, `data-mongodb-reactive`, `data-elasticsearch`
> **Связано:** `docs/spring-boot-starters-reference.md` (официальные стартеры Flyway/Liquibase), `docs/tree-variants.md`
