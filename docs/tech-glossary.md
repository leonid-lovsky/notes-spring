# Глоссарий технологий вне Spring-стека

> **Назначение:** краткие определения технологий за пределами Spring-стека — от инструментов разработки до production-инфраструктуры и архитектурных паттернов
> **Когда читать:** при работе с конкретной технологией из списка (Kubernetes, Terraform, Jenkins, SonarQube, AWS, Redis, Kafka, Elastic, OAuth2, jMolecules, Axon и др.)
> **Статус:** справочник, большинство технологий в проекте не используется; Axon/jMolecules/Kafka — на изучение в будущем
> **Разделы:** плоский список «технология — определение — статус в проекте»
> **Связано:** `docs/microservices-reference.md` → «Где реализуются технологии», CLAUDE.md → «Технологии»
...
- **testcontainers** — интеграционные тесты через реальные зависимости (БД, брокеры) в Docker-контейнерах вместо моков. Convention-плагины построены (`spring-boot-testcontainers`+`-testcontainers-mongodb`), но пока не применяются ни в одном модуле — см. `docs/decisions-log.md` → «Технологическая и вендорная ось адаптеров»
