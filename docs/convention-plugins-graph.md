...
> Новый 2026-08-01, снимок ТЕКУЩЕГО состояния (не история решений — история и обоснования «почему» остаются в [decisions-log.md](decisions-log.md), искать по разделу «Convention plugins — принцип именования и структура»). Причина создания — граф разбросан по нескольким хронологическим правкам decisions-log.md, реконструировать текущее состояние оттуда занимает больше времени, чем прочитать один плоский список. Обновлять при каждом добавлении/удалении/переименовании convention-плагина или изменении его `plugins{}`-блока — в той же правке, не откладывая. Уже пережил несколько раундов правок в день создания — см. «Атомизация базовых плагинов» в decisions-log.md → `base`→`java`, `library`→`java-library`, `reactor`→`project-reactor`, `codequality`-агрегатор инлайнирован, добавлены `spring-boot-validation` и `spring-boot-actuator` (воссоздан).
...
- `com.example.spring-cloud` — `spring-boot` — + Spring Cloud BOM (`spring-cloud-dependencies`) — 0 прямых применений (родитель 6 плагинов ниже)

## Уровень 3 — родитель `spring-cloud` (5 плагинов) и bootable-схождение `spring-cloud-application`
...
- `com.example.spring-cloud-application` — **2 прямых родителя** `spring-cloud` + `spring-boot-application` (диамант, сходятся в `spring-boot` — доказанно безопасен, устранён и затем осознанно восстановлен пользователем 2026-07-15, см. decisions-log.md → «Диамант устранён, затем... в тот же день осознанно восстановлен») — сам не добавляет зависимостей, только композиция — 0 прямых применений (родитель 4 плагинов ниже)

## Уровень 4 — родитель `spring-cloud-application` (standalone bootable-плагины)

- `com.example.spring-cloud-config-server` — `spring-cloud-application` — + `spring-cloud-config-server` — 1 применение (`config/application`)
- `com.example.spring-cloud-eureka-server` — `spring-cloud-application` — + `spring-cloud-starter-netflix-eureka-server` — 1 применение (`registry/application`)
- `com.example.spring-cloud-gateway-webflux` — `spring-cloud-application` — + `spring-cloud-starter-gateway-server-webflux` — 1 применение (`gateway/application`)
- `com.example.spring-cloud-gateway-webmvc` — `spring-cloud-application` — + `spring-cloud-starter-gateway-server-webmvc` — 0 применений (альтернатива webflux-варианту в gateway, не выбрана)
...
Одна ось — BOM-цепочка (`java → java-library/reactor/spring-boot → spring-cloud → tech-плагин`, всегда 1 родитель по построению) и одна ортогональная — bootable (`org.springframework.boot`, второй `id(...)` там, где нужен `bootJar`: `spring-boot-application`, `spring-cloud-application` косвенно, `docker-compose` напрямую). Единственное реальное схождение путей внутри самого convention-плагина (вид 1) во всём графе — `spring-cloud-application` (уровень 3) — задокументировано, обсуждено, оставлено осознанно. Композиция 2+ convention-плагинов НА ЛИСТОВОМ модуле (вид 2 — например `application-h2-jpa` — `spring-boot-application`+`spring-boot-actuator`+`spring-boot-database-h2`, `webmvc`/`webflux`/`data-jpa` — своя технология + `spring-boot-validation`, `gateway/application` — 3 `spring-cloud-*` + `spring-boot-actuator`, или `application-mongodb` — `spring-boot-application`+`spring-boot-actuator`+`spring-boot-testcontainers`+`spring-boot-testcontainers-mongodb`+`spring-boot-docker-compose`) — не диамант для устранения, а штатный способ использования системы (см. CLAUDE.md → «Правила» → «Кольцевые/ромбовидные зависимости» → вид 2).
