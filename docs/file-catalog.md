...
Наглядный ASCII-снимок структуры каталогов («дерево проекта») — вынесен в отдельный файл `docs/tree-project.md` 2026-09-22; этот файл (`file-catalog.md`) остаётся авторитетным построчным списком со статусами.
...
### docs/ (15 файлов, перенесены из корня 2026-07-24 — реструктуризация документации под 100%-доступность для ИИ-агента, см. CLAUDE.md → «Правила» → «Документация — только для ИИ-агента, не для человека»)
...
- `docs/spring-data-repository-reference.md` — [REVIEW] — новый 2026-09-22, дерево репозиториев Spring Data по вендорам проекта (JDBC/R2DBC/MongoDB sync+reactive + JPA для сравнения), собран по запросу пользователя в ходе разбора «почему у JDBC нет своего интерфейса репозитория»
...
- `docs/user-note-access-model.md` — [REVIEW] — новый 2026-09-25, модель доступа `UserNote` (права на заметку, роли-шаблоны для UI, инварианты, хранение SQL/NoSQL, JWT/Spring Security, механизмы инвариантов по слоям UI/Framework/Model/Storage, итоговая модель кодом, вопрос «доступ потерян у всех») — ход обсуждения и точка остановки, не решение, по прямому запросу пользователя
...
- `gradle/checkstyle/checkstyle.xml` — [REVIEW] — 2026-09-25 переименован из `google_checks.xml` (путь задан явно `configFile` в `com.example.codequality-checkstyle`); копия Google Checks с переопределениями `LineLength`/`Indentation`, без `MissingJavadocType`/`MissingJavadocMethod` (удалены 2026-09-25)
...
- `build-logic/convention/build.gradle.kts` — [REVIEW] — 2026-09-06: classpath-зависимости PMD/SpotBugs добавлены (CPD добавлен и в тот же вечер убран — de.aaschmid.cpd не обновлялся 2 года, неофициальная интеграция, не прошёл критерий «стандартный инструмент»)
...
- `build-logic/com.example.spring-boot-test-h2.gradle.kts` — [REVIEW] — новый 2026-09-21, `testRuntimeOnly(h2)`: встроенная БД для `@DataJdbcTest`, применён в `data-jdbc`; test-scope, чтобы H2 не утекала в вендорные листья
- `build-logic/com.example.spring-boot-test-r2dbc-h2.gradle.kts` — [REVIEW] — новый 2026-09-21, `testRuntimeOnly(r2dbc-h2)`: встроенная БД для `@DataR2dbcTest`, применён в `data-r2dbc`; зеркальная пара к `spring-boot-test-h2`
...
- `build-logic/com.example.java-library.gradle.kts` — [REMOVED] — удалён 2026-09-24, нигде не применялся после удаления domain-слоя 2026-08-29 (был нужен только `contract`/`contract-reactive` × 3 сервиса ради `api(projects.*.domain)`, того domain больше нет); ранее переименован 2026-08-01 из `library.gradle.kts`
- `build-logic/com.example.javaformat.gradle.kts` — [REMOVED]
- `build-logic/com.example.codequality-jacoco.gradle.kts` — [REVIEW] — применялся напрямую из `java.gradle.kts` 2026-08-01→2026-09-24, с 2026-09-24 снова через `codequality`-агрегатор (воссоздан, см. запись выше)
- `build-logic/com.example.codequality-jacoco-report-aggregation.gradle.kts` — [REVIEW] — применялся напрямую из `java.gradle.kts` 2026-08-01→2026-09-24, с 2026-09-24 снова через `codequality`-агрегатор
- `build-logic/com.example.codequality.gradle.kts` — [REVIEW] — воссоздан 2026-09-24 по прямому запросу пользователя, тот же паттерн, что был убран 2026-08-01 («чистый список id без своей конфигурации, единственный потребитель») — сегодня агрегирует 8 `codequality-*` (было 5), единственный потребитель по-прежнему `com.example.base`
- `build-logic/com.example.codequality-checkstyle.gradle.kts` — [REVIEW] — применялся напрямую из `java.gradle.kts` 2026-08-01→2026-09-24, с 2026-09-24 снова через `codequality`-агрегатор; с 2026-09-04 без `spring-javaformat-checkstyle`-зависимости, с 2026-09-25 `configFile` указывает на `gradle/checkstyle/checkstyle.xml` и с `maxWarnings = 0`
