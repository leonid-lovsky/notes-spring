...
- `build-logic/com.example.codequality-checkstyle.gradle.kts` — [REVIEW] — применялся напрямую из `java.gradle.kts` 2026-08-01→2026-09-24, с 2026-09-24 снова через `codequality`-агрегатор; с 2026-09-04 без `spring-javaformat-checkstyle`-зависимости, `configFile` указывает на `gradle/checkstyle/google_checks.xml` (см. `gradle/checkstyle/`)
