...
- `user-note/data-jdbc/src/test/java/com/example/usernote/UserNoteRepositoryTests.java` — [REVIEW] — smoke без Spring-аннотации, пустой `smoke()` (2026-09-21): `@DataJdbcTest`/`@DataR2dbcTest` не стартуют — в модуле нет драйвера БД (вендор — граница листа), JDBC падает `Failed to replace DataSource with an embedded database`, R2DBC — `Failed to determine a suitable R2DBC Connection URL`
...
- `user-note/data-mongodb/src/test/java/com/example/usernote/UserNoteRepositoryTests.java` — [REVIEW] — smoke: `@DataMongoTest` + вложенный `@SpringBootApplication static class SliceApplication` + пустой `contextLoads()` (2026-09-21); одна аннотация и для blocking, и для reactive репозитория
...
- `user-note/data-mongodb-reactive/src/test/java/com/example/usernote/UserNoteRepositoryTests.java` — [REVIEW] — smoke: `@DataMongoTest` + вложенный `@SpringBootApplication static class SliceApplication` + пустой `contextLoads()` (2026-09-21); одна аннотация и для blocking, и для reactive репозитория
...
- `user-note/data-r2dbc/src/test/java/com/example/usernote/UserNoteRepositoryTests.java` — [REVIEW] — smoke без Spring-аннотации, пустой `smoke()` (2026-09-21): `@DataJdbcTest`/`@DataR2dbcTest` не стартуют — в модуле нет драйвера БД (вендор — граница листа), JDBC падает `Failed to replace DataSource with an embedded database`, R2DBC — `Failed to determine a suitable R2DBC Connection URL`
...
- `user-note/controller-webmvc/src/main/java/com/example/usernote/UserNoteController.java` — [REVIEW] — пустой класс-заглушка
...
- `user-note/controller-webmvc/src/test/java/com/example/usernote/UserNoteControllerTests.java` — [REVIEW] — smoke: `@WebMvcTest` + вложенный `@SpringBootApplication static class SliceApplication` (в модуле нет своего `@SpringBootApplication` — без него слайс падает `Unable to find a @SpringBootConfiguration`) + пустой `contextLoads()`; имя вложенного класса не должно начинаться на `Test` — PMD `TestClassWithoutTestCases` (2026-09-21)
...
- `user-note/controller-webflux/src/main/java/com/example/usernote/UserNoteController.java` — [REVIEW] — пустой класс-заглушка
...
- `user-note/controller-webflux/src/test/java/com/example/usernote/UserNoteControllerTests.java` — [REVIEW] — smoke: `@WebFluxTest` + вложенный `@SpringBootApplication static class SliceApplication` + пустой `contextLoads()` (2026-09-21)
