...
class UserNoteMongoMapperImpl implements UserNoteMongoMapper {
    ...
    public UserNoteDocument toDocument(UserNoteRequest request) {
        return new UserNoteDocument(
                new UserNoteKey(request.userId(), request.noteId()), request.role());
    }

    @Override
    public UserNoteResponse toResponse(UserNoteDocument document) {
        return new UserNoteResponse(
                document.getId().getUserId(), document.getId().getNoteId(), document.getRole());
    }
}
