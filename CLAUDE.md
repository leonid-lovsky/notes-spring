...
> Последнее обновление: Tue Jul 14 13:00:38 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **Jakarta Bean Validation для Entity/Document — стартер подключён, аннотации не расставлены** (было «принято к реализации, не начато» до 2026-07-14): `spring-boot-starter-validation`(`-test`) добавлен в `com.example.spring-boot` (см. «⚡ Задачи») — доступен теперь во всех Spring Boot модулях без отдельного opt-in плагина. Не сделано: `jakarta.validation.constraints.*` (`@NotNull`/`@NotBlank` и т. п.) по-прежнему не расставлены ни на одном из 15 `Entity`/`Document`-классов, `@Valid` не используется ни в одном контроллере/адаптере — сам механизм присутствует в classpath, но не применяется. Отдельный от JSpecify/NullAway механизм: там — статический null-checking на этапе компиляции, тут — runtime-валидация (`@Valid`/`Validator`) — не взаимозаменяемы, дополняют друг друга. Не реализовывать расстановку аннотаций до отдельного запроса
...
#### user-note/data-jdbc/ (10 файлов, было 6 — 2026-07-14, переход на Spring Data JDBC repository, см. «⚡ Задачи»)
...
- `user-note/data-jdbc/src/main/java/com/example/usernote/data/jdbc/model/UserNoteJdbcEntity.java` — [REVIEW] — новый (2026-07-14): `@Table("user_notes")`, `@Id private @Nullable UUID id`, `@Column("user_id")`/`@Column("note_id")`/`@Column("role")` (`role` как `String`, не enum — согласуется с открытым вопросом «role как String в R2DBC/JDBC») — структура 1:1 с `UserNoteR2dbcEntity`; unique constraint на `user_id+note_id` не создаётся (как и у `data-r2dbc`) — см. «Управление схемой для R2DBC/JDBC»
...
- `user-note/data-jdbc/src/main/java/com/example/usernote/data/jdbc/mapper/UserNoteJdbcMapper.java` — [REVIEW] — переписан 2026-07-14: `UserNoteRole.valueOf(...)`/`.name()` ручная конвертация enum↔String сохранена (открытый вопрос «role как String в R2DBC/JDBC» не решён, просто перенесён из `fromRow` в `toResponse`/`toNewEntity`); структура идентична `UserNoteR2dbcMapper`
...
- `user/data-jdbc/src/main/java/com/example/user/data/jdbc/model/UserJdbcEntity.java` — [REVIEW] — новый (2026-07-14): `@Table("users")`, `@Id private @Nullable UUID id`, `@Column("username")`/`@Column("email")` — структура 1:1 с `UserR2dbcEntity`; unique constraint на `username`/`email` не создаётся (как и у `data-r2dbc`, `spring-data-relational` не поддерживает constraint-атрибуты) — см. «Управление схемой для R2DBC/JDBC»
...
#### build-logic/convention/src/main/kotlin/ — precompiled script plugins (35 файлов; было 36 — `com.example.spring-boot-actuator.gradle.kts` удалён 2026-07-14, до этого 37 — `com.example.javaformat.gradle.kts` удалён)
