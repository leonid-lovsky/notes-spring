...
> Последнее обновление: Fri Jul 24 08:41:09 UTC 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **⚠️ Sandboxed/remote-агент окружение может не уметь запустить сборку вообще** (2026-07-24, обнаружено при работе через Claude Code on the web): проект требует Java 25 (`.java-version`), но в некоторых remote execution environment физически установлена только JDK 21, а автозагрузка нужного тулчейна через Gradle toolchain resolver `foojay` (`api.foojay.io`) заблокирована сетевой политикой окружения (`CONNECT tunnel failed, 403` — так же падает и прямой `curl` на `api.foojay.io`/`start.spring.io`, не только Gradle). Симптом — `./gradlew` падает на **любой** задаче, требующей toolchain (`compileJava`, `dependencies`, `check`, `build`), включая полностью нетронутые модули (`note:domain` и т. п.) — это диагностический признак, что проблема в окружении, а не в конкретной правке. Перед тем как делать вывод «правка сломала сборку» — обязательно проверить `./gradlew :{нетронутый-модуль}:compileJava` отдельно: если падает тем же способом — билд-верификация в этом окружении недоступна в принципе, откладывать её до среды с нужной JDK и сетевым доступом (локальная машина пользователя, CI), коммитить по явному решению пользователя без неё, а не блокировать работу
