...
> Последнее обновление: Thu Jul 23 22:32:43 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **Симметрия ArchUnit + стратегия покрытия матрицы адаптер×драйвер тестами** (2026-07-23, поднято пользователем в приоритет параллельно с построчным пересмотром `file-catalog.md`; **не реализовывать до отдельного запроса**) — два связанных вопроса без решения:
  ...
  2. Матрица тестового покрытия — по состоянию на сейчас (см. пункт выше про Testcontainers) на сервис: `jpa`/`jdbc`/`r2dbc` × (H2/PostgreSQL/MySQL) + `mongodb` + `mongodb-reactive` = 11 комбинаций × 3 сервиса = 33, H2 покрыт исходным `contextLoads()`, 8 × 3 = 24 — новыми explicit-классами. Варианты на будущее при разрастании: (A) отдельный класс на комбинацию (текущий подход) — явно, IDE-дружественно, линейный рост файлов; (B) параметризация через JUnit 5 `@ParameterizedTest`/переиспользуемый test-фикстура-код в `src/test/java` каждого сервиса (не `build-logic/` — правило «без shared-функций» относится к convention-плагинам, не к тестовому коду модуля) — меньше файлов, но менее explicit по каждому вендору; (C) мета-тест по типу ArchUnit, проверяющий «на каждый вендорный convention-плагин в модуле должен существовать соответствующий `*ApplicationTests`-класс» — ловит рассинхронизацию автоматически, но зависит от решения по пункту 1
