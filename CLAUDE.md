...
> Последнее обновление: 2026-06-20T21:40Z
...
- **Нужен ли отдельный `service/`** — для простого CRUD `service/` делегирует без логики; можно реализовать `*UseCase` прямо в `data-jpa/` адаптере и убрать `service/`; оправдан только когда use case координирует несколько репозиториев или содержит нетривиальную бизнес-логику
