...
> Последнее обновление: 2026-06-23T19:51Z
...
2. **Именование input port → use case implementation**
   - `domain/`: `NoteUseCase` (input port, интерфейс)
   - `service/`: `NoteService`? — согласована ли стратегия?

3. **Сигнатуры методов и возвращаемые типы**
   - `domain/Repository`: семантика коллекции (Evans) частично принята — но не систематизирована
   - `domain/UseCase`: что возвращают мутирующие операции? нет стратегии
   - Нет единого правила: когда `void`, когда доменный объект, когда `Optional`

4. **Ответственность и состав методов**
   - Граница между `Repository` и `UseCase` размыта: кто отвечает за валидацию существования? кто за бизнес-правила?
   - Нет стратегии: один `UseCase` на сервис или один на операцию (Uncle Bob)?
