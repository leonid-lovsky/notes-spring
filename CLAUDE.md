...
> Последнее обновление: Wed Jul 15 00:13:16 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- Диамант у 4 standalone `spring-cloud-*`-плагинов (`config-server`/`eureka-server`/`gateway-webflux`/`gateway-webmvc`) устранён — новый линейный плагин `com.example.spring-cloud-application` (родитель `spring-boot-application` вместо параллельных `spring-cloud`+`spring-boot-application`). Диамант был доказанно безопасен (идемпотентность Gradle `PluginManager`, баг gradle/gradle#13252 исключён грепом), устранён по прямому требованию пользователя — правило «Кольцевые/ромбовидные зависимости» в «Правила» ужесточено: отслеживать и сообщать всегда, безопасность не повод оставить как есть. Без shared-функций (явный отказ пользователя) — только композиция `id(...)`
...
jspecify        — id("java-library") + api("org.jspecify") (2026-07-15, вынесен из nullaway
                  по прямому запросу пользователя); следствие: все модули транзитивно получают
                  java-library через base → codequality → jspecify
...
└── spring-boot             — io.spring.dependency-management + Spring Boot BOM
    ├── spring-boot-*        (16 технологических плагинов, без spring-boot-application ниже)
    ├── spring-boot-application — + id("org.springframework.boot") + actuator (bootable-ось)
    │   └── spring-cloud-application — + BOM spring-cloud-dependencies (линейная цепочка через
    │       │                           bootable-ось, не диамант)
...
**Диамант устранён 2026-07-15** (был: 4 standalone `spring-cloud-*`-плагина применяли двух прямых родителей — `spring-cloud` и `spring-boot-application` — оба сходились в общем предке `spring-boot`). Технически безопасно (идемпотентность Gradle `PluginManager`; известное исключение gradle/gradle#13252 про cross-project apply — исключено грепом, в convention-плагинах нет ни одного `rootProject.pluginManager`/`project(":...")`-вызова), но устранено по прямому требованию пользователя (см. правило «Кольцевые/ромбовидные зависимости» в «Правила» — отслеживать и сообщать всегда, даже про безопасные). Решение — новый `com.example.spring-cloud-application`: точная копия `com.example.spring-cloud` (тот же импорт Spring Cloud BOM через `libs.findVersion("spring-cloud")`), но с родителем `spring-boot-application` вместо `spring-boot`. Каждый из 4 файлов применяет теперь единственного прямого родителя — граф строго линеен. `com.example.spring-cloud`/`com.example.spring-boot-application` не менялись, остальные 5 `spring-cloud-*`-плагинов и 17 модулей с одиночным `spring-boot-application` не затронуты. Без shared-функций/helper-кода — пользователь явно отказался от этого варианта, только композиция через `id(...)`. `com.example.spring-boot-application` несёт `spring-boot-starter-actuator`(`-test`) — actuator привязан к bootable-оси, не к BOM-цепочке. `com.example.javaformat` удалён 2026-07-13 (см. «Задачи») — конфигурация перенесена в `com.example.checkstyle` без правок по модулям-потребителям.
