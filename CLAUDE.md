...
> Последнее обновление: Thu Jul 23 20:48:19 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **Вендор БД** (H2/PostgreSQL/MySQL для jpa/jdbc/r2dbc — код `data-jpa`/`data-jdbc`/`data-r2dbc` идентичен для любого вендора) — НЕ отдельные модули: 4 новых convention-плагина (`spring-boot-postgresql-database`, `-mysql-database`, `-r2dbc-postgresql-database`, `-r2dbc-mysql-database`) кладут драйверы `runtimeOnly` (зеркалят `spring-boot-h2-database`/`-r2dbc-h2-database`, без версии — Spring Boot BOM), активный вендор — `spring.profiles.active=postgresql|mysql` + `application-{vendor}.properties` (`spring.datasource.url`/`spring.r2dbc.url`). Без активного профиля — H2 in-memory, как раньше (`DataSourceAutoConfiguration` пробует фиксированный embeddable-enum H2/Derby/HSQLDB, не путается от лишних драйверов на classpath) — явный `application-h2.properties` осознанно не создан
- **MongoDB не имеет embedded-аналога H2** — `spring-boot-testcontainers` (нейтральная обвязка: `spring-boot-testcontainers`+`testcontainers-junit-jupiter`, годится для любого будущего Testcontainers-модуля) + `spring-boot-testcontainers-mongodb` (только `testcontainers-mongodb`-артефакт — композиция двух `id(...)` в leaf, не единый комбинированный плагин; исходно был один плагин, разделён в тот же день по прямому замечанию пользователя о нарушении композиционного принципа) — тесты, `@ServiceConnection MongoDBContainer` из пакета `org.testcontainers.mongodb` (сменился с `org.testcontainers.containers` в текущей версии Testcontainers, проверено по docs.spring.io) + `spring-boot-docker-compose` (родитель `spring-boot-application` — `developmentOnly` требует bootable-плагин; `compose.yaml` с `mongo:8` в каждом Mongo-модуле) — `bootRun` автоматически поднимает контейнер без ручного `docker run`
- Верифицировано вживую (не только `clean check`): `application-jpa` без профиля — H2; с `--spring.profiles.active=postgresql` — Hikari реально подключился к `jdbc:postgresql://localhost:5432/notedb` (`PostgreSQLDialect`); `application-mongodb:bootRun` — Docker Compose поднял `mongo:8`, приложение стартовало и подключилось; `application-mongodb:test` — Testcontainers поднял контейнер, `contextLoads()` прошёл
- Vendor-подключение (PostgreSQL/MySQL) не покрыто автотестами/CI — только ручная проверка с локальным контейнером; при необходимости — отдельная Testcontainers-интеграция на вендорную ось (не сделано)
