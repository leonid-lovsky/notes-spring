...
> Последнее обновление: Fri Jul 24 10:23:45 UTC 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
ГОТОВО   note/ · user/ · user-note/ — domain · contract · contract-reactive · webmvc · webflux
                  ...
                  application-{jpa,jdbc,r2dbc}-{h2,mysql,postgresql} · application-mongodb ·
...
application-jpa-{h2,mysql,postgresql}/     composition root (sync)      → webmvc + data-jpa
application-jdbc-{h2,mysql,postgresql}/    composition root (sync)      → webmvc + data-jdbc
application-r2dbc-{h2,mysql,postgresql}/   composition root (reactive)  → webflux + data-r2dbc
...
- **Composition-root главный класс** (2026-07-23, расширено вендорным суффиксом 2026-07-24) — `{Service}{Tech}Application` в `application-{tech}/` для Mongo (`NoteMongoApplication`, `NoteMongoReactiveApplication`); `{Service}{Tech}{Vendor}Application` в `application-{tech}-{vendor}/` для jpa/jdbc/r2dbc (`NoteJpaH2Application`, `NoteJpaMysqlApplication`, `NoteJpaPostgresqlApplication`, ...) — вендор всегда явный суффикс, без неявного «без суффикса = H2» (см. `decisions-log.md` → «Вендор БД — граница Gradle-модуля»). Тестовый класс — `...ApplicationTests` по тому же паттерну. До 2026-07-23 было 2 модуля на сервис (`application`/`application-reactive`) без Tech-суффикса
...
**Vendor** (2026-07-24, только для composition-root класса/модуля jpa/jdbc/r2dbc, см. выше): `H2` · `Mysql` · `Postgresql` — всегда суффикс после `{Tech}`, порядок `{Tech}{Vendor}` (`JpaH2`, не `H2Jpa`) — технология доступа архитектурно более фундаментальна, вендор — конфигурация поверх неё
