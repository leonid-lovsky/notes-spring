...
> Последнее обновление: Thu Jul 23 20:40:10 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
└── spring-boot             — io.spring.dependency-management + Spring Boot BOM
    ├── spring-boot-*        (21 технологический плагин, без spring-boot-application ниже —
    ...
    │                         + testcontainers-mongodb; docker-compose — родитель
...
- **MongoDB не имеет embedded-аналога H2** — `spring-boot-testcontainers-mongodb` (тесты, `@ServiceConnection MongoDBContainer` из пакета `org.testcontainers.mongodb` — сменился с `org.testcontainers.containers` в текущей версии Testcontainers, проверено по docs.spring.io) + `spring-boot-docker-compose` (родитель `spring-boot-application` — `developmentOnly` требует bootable-плагин; `compose.yaml` с `mongo:8` в каждом Mongo-модуле) — `bootRun` автоматически поднимает контейнер без ручного `docker run`
