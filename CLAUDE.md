...
> Последнее обновление: Thu Jul 23 21:01:13 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **MongoDB не имеет embedded-аналога H2** — `spring-boot-testcontainers` (нейтральная обвязка: `spring-boot-testcontainers`+`testcontainers-junit-jupiter`, годится для любого будущего Testcontainers-модуля) + `spring-boot-testcontainers-mongodb` (только `testcontainers-mongodb`-артефакт — композиция двух `id(...)` в leaf, не единый комбинированный плагин; исходно был один плагин, разделён в тот же день по прямому замечанию пользователя о нарушении композиционного принципа; подтверждено через docs.spring.io — один `MongoDBContainer`/`MongoConnectionDetails` общий для sync и reactive клиента, Spring Boot сам выбирает тип по classpath) — тесты, `@ServiceConnection MongoDBContainer` из пакета `org.testcontainers.mongodb` (сменился с `org.testcontainers.containers` в текущей версии Testcontainers, проверено по docs.spring.io) + `spring-boot-docker-compose` (родитель `spring-boot` + прямой `id("org.springframework.boot")`, как у `spring-boot-application`, не через него как родителя — тоже исправлено по замечанию о композиции; `developmentOnly` требует bootable-плагин; `compose.yaml` с `mongo:8` в каждом Mongo-модуле) — `bootRun` автоматически поднимает контейнер без ручного `docker run`
