...
> Последнее обновление: Mon Jul 13 14:25:29 IDT 2026 **Всё временно** — любое решение подлежит обсуждению и изменению.
...
- **Персональные правила форматирования vs Spring JavaFormat** — пользователь сформулировал 10 правил, часть из которых расходится с текущим `io.spring.javaformat`/`checkstyle.xml`: неограниченная длина строки и отключение авто-переноса при форматировании, никогда не пустая строка перед закрывающей скобкой, всегда `\n` после последней закрывающей скобки, ровно одна пустая строка между методами, не более одной пустой строки подряд в файле, аннотации перед полями/классами/интерфейсами/методами — всегда каждая на своей строке, аннотации перед параметрами — никогда не переносятся, отступ — 4 пробела без табов (уже и так конвенция проекта, см. `.springjavaformatconfig`). Решено отложить кодирование в `gradle/checkstyle/checkstyle.xml`/`build-logic` на потом — не реализовывать до отдельного запроса
- **Порядок методов реализации = порядок объявления в интерфейсе** — отдельное, более сложное правило (см. выше): должен быть жёстко проверяемым, но готового Checkstyle-правила под него нет — нужна семантическая привязка к конкретному реализуемому интерфейсу, которой синтаксическому Checkstyle не хватает без резолвинга типов. Кандидат — кастомный Error Prone `BugChecker` (в проекте уже есть `net.ltgt.errorprone`/NullAway инфраструктура, а Error Prone в отличие от Checkstyle работает поверх резолвленных `javac`-символов) вместо кастомного Checkstyle Check. Не реализовывать до отдельного запроса
