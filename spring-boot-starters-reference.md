...
> Вынесено из CLAUDE.md 2026-07-14 (см. [CLAUDE.md](CLAUDE.md) → «Правила» → лимит размера файла).

**Список ниже — дословная копия с Spring Initializr («select all», объединены несколько присланных снапшотов, версии — старшие из встретившихся). Не удалять, не сокращать, не менять строки списка ни при какой задаче, включая явное сокращение объёма CLAUDE.md — источник истины при проверке полноты convention-плагинов на пропущенные `-test`-компаньоны/plain-API-варианты. Комментарии/пояснения вокруг списка редактировать можно, сам список — нет; допустима только проверка на дубликаты строк (2026-07-14: найден и убран один случайный дубль `testImplementation 'org.springframework.boot:spring-boot-starter-ldap'` рядом с корректным `-ldap-test`, тем же паттерном, что и ранее найденный дубль `spring-security-messaging`).**
